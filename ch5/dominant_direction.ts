...
function dominantDirection(text: string): string {
    let counts = countBy(text, (char: string) => {
...
function repeat(n : number, action: (n: number) => void) {
  for (let i = 0; i < n; i++) {
    action(i);
  }
}
...
function characterScript(code: number) {
  for (let script of SCRIPTS) {
    if (script.ranges.some(([from, to] : Array<number>) => {
...
function countBy(items: string, groupName: (item: string) => string) {
  let counts = [];
...
function textScripts(text: string) {
